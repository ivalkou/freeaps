...
protocol HealthKitManager: GlucoseSource, CarbsSource {
    ...
    /// Check availability to save data of Carbs type to Health store
    func checkAvailabilitySaveCarbs() -> Bool
...
final class BaseHealthKitManager: HealthKitManager, Injectable {
    private enum Config {
        // unwraped HKObjects
        static var permissions: Set<HKSampleType> { Set([healthBGObject, healthCarbObject].compactMap { $0 }) }
    ...
    func checkAvailabilitySaveCarbs() -> Bool {
        Config.healthCarbObject.map { checkAvailabilitySave(objectTypeToHealthStore: $0) } ?? false
    }
